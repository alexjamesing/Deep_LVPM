...
class SiameseVicReg(tf.keras.Model):
    ...
    def add_DLVPM_layer(self, model, regularizer):
        ...
        if isinstance(model, tf.keras.Sequential):
            ...
            if self.orthogonalization == 'Moore-Penrose':
                model.add(tf.keras.layers.Dense(self.ndims,kernel_regularizer=regularizer))
            ...
            if self.orthogonalization == 'Moore-Penrose':
                input = model.input
                x = tf.keras.layers.Dense(self.ndims,kernel_regularizer=regularizer)(model.output)
    ...
    def train_step(self, inputs):
        ...
        for vie in range(len(self.model_list)):
            ...
            with tf.GradientTape() as tape:
                ...
                y_true =  tf.squeeze(tf.gather(y,tf.where(self.Path[vie,:]),axis=2),axis=3)
    ...
    def mse_loss(self, y1, y2, lambda_var=1, lambda_cov=1, lambda_invar=1):
        """
        Implementation of the VICReg loss function.
        (self, y1, y2, lambda_var=1, lambda_cov=1e-4, lambda_invar=25)
        Parameters:
        - y1, y2: Tensor, embeddings of two augmented versions of the same batch of images.
        - lambda_var: Weight of the variance loss component.
        - lambda_cov: Weight of the covariance loss component.
        - lambda_invar: Weight of the invariance loss component.
        
        Returns:
        - total_loss: Scalar, the total VICReg loss.
        """
        # Invariance Loss
        ...
        invariance_loss = tf.reduce_mean(tf.norm(y1 - y2, axis=-1))
        
        # Variance Loss (ensure variance is not too low)
        std_y1 = tf.math.reduce_std(y1, axis=0) + 1e-6
        std_y2 = tf.math.reduce_std(y2, axis=0) + 1e-6
        variance_loss = tf.reduce_mean(tf.maximum(0., 1. - std_y1)) + tf.reduce_mean(tf.maximum(0., 1. - std_y2))
        
        # Covariance Loss (off-diagonal elements)
        
        def covariance_loss(embeddings):
            
            embeddings = tf.cast(embeddings, tf.float32)
            batch_size = tf.shape(embeddings)[0]
            feature_size = tf.shape(embeddings)[1]
            
            # Cast the result of the division to tf.float32 to ensure type consistency
            normalization_factor = tf.cast(1 / (batch_size - 1), tf.float32)
            
            covariance_matrix = normalization_factor * tf.matmul(tf.transpose(embeddings), embeddings)
            covariance_matrix -= tf.eye(feature_size, dtype=tf.float32)  # Ensure tf.eye() is also float32
            return tf.reduce_sum(tf.pow(covariance_matrix, 2))
                
        cov_loss_y1 = covariance_loss(y1)
        cov_loss_y2 = covariance_loss(y2)
        
        total_loss = (lambda_invar * invariance_loss + 
                    lambda_var * variance_loss +
                    lambda_cov * (cov_loss_y1 + cov_loss_y2))
