...
Ensure that :mod:`deep_lvpm` is installed as described on the :doc:`installation` page and set ``KERAS_BACKEND=tensorflow`` so Keras loads the TensorFlow backend.  The tutorial uses the small sample datasets bundled with the package under ``deep_lvpm.data`` and can be run on CPU.
...
Training proceeds with the standard Keras ``fit`` interface.  The Keras 3 ``evaluate`` method returns a dictionary with four entries (``total_loss``, ``cross_metric``, ``mse_loss``, and ``redundancy``) summarising performance across views.
...
.. code-block:: python
   ...
   metrics = DLVPM_Structural_instance.evaluate(X_arr)
   print({name: float(value) for name, value in metrics.items()})

6. Evaluate on the test set
---------------------------

We load the separate test dataset and compute the evaluation metrics for the learned DLVs (again returned as a dictionary).
...
.. code-block:: python
   ...
   metrics_test = DLVPM_Structural_instance.evaluate(X_arr_test)
   print({name: float(value) for name, value in metrics_test.items()})
