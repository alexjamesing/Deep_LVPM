...
#     """Convert images to grayscale with probability `prob` during training.

#     Expects inputs shaped (B, H, W, 3) in [0, 1]. At inference, returns inputs unchanged.
#     """
#     def __init__(self, prob: float = 0.2, **kwargs):
#         super().__init__(**kwargs)
#         self.prob = float(prob)

#     def call(self, inputs, training=None):
#         if training is False or self.prob <= 0.0:
#             return inputs
#         x = tf.convert_to_tensor(inputs)
#         # Per-sample mask
#         b = tf.shape(x)[0]
#         mask = tf.random.uniform((b, 1, 1, 1), dtype=x.dtype) < self.prob
#         # Convert to grayscale and tile back to 3 channels
#         gray = tf.image.rgb_to_grayscale(x)
#         gray3 = tf.concat([gray, gray, gray], axis=-1)
#         return tf.where(mask, gray3, x)
...
#         layers.Lambda(
#             lambda x: tf.where(
#                 tf.random.uniform([tf.shape(x)[0], 1, 1, 1]) < 0.5,
...
# # Lighten weight decay to preserve variance in correlation-based training
# WEIGHT_DECAY = 1e-4
# NDIMS = 512

# def build_simple_conv_encoder():
#     inputs = keras.Input(shape=INPUT_SHAPE)
#     # Simple ConvNet: 32 -> 16 -> 8 spatially
#     x = layers.Conv2D(
#         32,
#         3,
#         padding="same",
...
#     # Global pooling and compact head to NDIMS
#     x = layers.GlobalAveragePooling2D()(x)
#     x = layers.Dense(1024, activation="relu")(x)
#     x = layers.BatchNormalization()(x)
#     outputs = layers.Dense(NDIMS, activation="relu")(x)
#     return keras.Model(inputs, outputs, name="cifar_simple_conv")


# CIFAR_image_model = build_simple_conv_encoder()
...
# optimizers = [
#     keras.optimizers.Adam(learning_rate=1e-5, clipnorm=1),
#     keras.optimizers.Adam(learning_rate=1e-5, clipnorm=1),
# ]
...
# test_dlvs = dlvpm_model.predict(test_ds)

# correlation_matrices = dlvpm_model.calculate_corrmat(test_dlvs)

# corrmean = [np.mean(a[0,1]) for a in correlation_matrices]
...
# Compile with branch-specific optimisers.
optimizers = [
    keras.optimizers.Adam(learning_rate=1e-4, clipnorm=1),
    keras.optimizers.Adam(learning_rate=1e-4, clipnorm=1),
]
dlvpm_model.compile(optimizers)
