...
"""Siamese CIFAR-10 with CLIP (TensorFlow backend)

This tutorial mirrors the Siamese CIFAR-10 example that used the
DLVPM `StructuralModel`, but swaps in the CLIP-style model
(`deep_lvpm.multi_model.CLIP`). We create two augmented views of
each image, train a shared-weight encoder with a linear projection
head via the CLIP loss, and then evaluate with a linear SVM.
"""
...
# One-hot labels for downstream compatibility (not used by CLIP)
...
# Shared encoder backbone (CLIP adds its own Dense projection head)
WEIGHT_DECAY = 0.0
NDIMS = 512  # CLIP projection size

# Hybrid ConvNet + Transformer backbone (mirrors the siamese TF tutorial)
...
def build_hybrid_backbone():
    ...
    # Pool tokens and produce backbone features (CLIP adds the NDIMS projection)
...
# Build CLIP siamese model with shared weights across the two views
...
clip_model.compile(opt_list)


# Train
EPOCHS = 10
history = clip_model.fit(train_ds, validation_data=val_ds, epochs=EPOCHS, verbose=True)


# Evaluate CLIP loss on held-out augmented pairs
test_ds = make_siamese_views_dataset(x_test, batch_size=BATCH_SIZE, shuffle=False, training=True)
eval_metrics = clip_model.evaluate(test_ds, verbose=True)
print("Evaluation metrics:", eval_metrics)


# Downstream linear evaluation using the learned CLIP projection
...
# The per-view sub-model outputs the (pre-normalised) CLIP embeddings
image_head = clip_model.model_list[0]

train_emb = image_head.predict(x_train, batch_size=64, verbose=1)
test_emb = image_head.predict(x_test, batch_size=64, verbose=1)

# Use L2-normalised embeddings for linear probing
...
print(f"\nSVM accuracy on CIFAR-10 test set (CLIP embeddings): {acc:.4f}\n")
